...
//LED
volatile int brightR = 0, brightG = 0, brightB = 0;
...
int channelSetTimer = 0, tvRemoconChannel = 0;
...
int  RFreceive()
{
  ...
  for (int i = 0; i < num; i++)
  {
    ...
    if (input >= 0)
    {
      ...
      if (_count > 6)    _header = 0, _count = 0;
      else
      {
...
{
  byte _ch;
  if (((_code == CH1_KEY_U) && (tvRemoconChannel != 1)) || ((_code == CH2_KEY_U) && (tvRemoconChannel != 2)) || ((_code == CH3_KEY_U) && (tvRemoconChannel != 3)) || ((_code == CH4_KEY_U) && (tvRemoconChannel != 4)))
  {
    if (channelSetFlag == 0)
    {
      if (_code == CH1_KEY_U)       channelSetFlag = CH1;
      else if (_code == CH2_KEY_U)  channelSetFlag = CH2;
      else if (_code == CH3_KEY_U)  channelSetFlag = CH3;
      else if (_code == CH4_KEY_U)  channelSetFlag = CH4;
    ...
    {
      if (channelSetFlag == CH1)         _ch = CH1;
      else if (channelSetFlag == CH2)    _ch = CH2;
      else if (channelSetFlag == CH3)    _ch = CH3;
      else if (channelSetFlag == CH4)    _ch = CH4;

      RemoconChannelSet(_ch);
      Serial.println(_ch);
...
void RemoconChannelSet(byte channel)
{
  EEPROM.write(EEP_REMOCON_CHANNEL, channel);
  tvRemoconChannel = channel;
}
...
void TestMode()
{
  ...
  byte ledSequence = 0, motorSequence = 0;
  int speed = 70;
  int bright = 20;
...
int ModeSelect()
{
  ...
  _val[0] = analogRead(SFL);
  _val[1] = analogRead(SFF);
  _val[2] = analogRead(SFR);

  if (((_val[0] < 400) ) || (_val[1] < 400) || (_val[2] < 400))
  {
    if (((_val[0] < 400) ) && (_val[2] < 400))  _mode = MAZE_BOT;

    else if (_val[0] < _val[1])
    {
      if (_val[0] < _val[2])   _mode = AVOID;
      else                     _mode = LINE_TRACER;
    }

    else
    {
      if (_val[1] < _val[2])  _mode = UNPLUGGED;
      else                    _mode = LINE_TRACER;
    }
